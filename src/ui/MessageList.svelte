...
<div class="ml-root" bind:this={scrollEl} on:scroll={onScroll} role="log" aria-live="polite">
  ...
  {#if messages.filter(m => m.role === "user" || m.role === "assistant").length === 0 && !streamBuf}
    <div class="ml-empty">
      ...
      <p class="ml-empty-title">{plugin.i18n.t("chat.empty")}</p>
      <p class="ml-empty-hint">{plugin.i18n.t("chat.emptyHint")}</p>
    </div>
  {/if}
  ...
  {#each messages as m (m)}
    {#if m.role === "user"}
      <div class="ml-turn ml-turn-user">
        <div class="ml-name ml-name-user">{plugin.i18n.t("chat.you")}</div>
        <div class="ml-content">{m.content}</div>
      </div>

    {:else if m.role === "assistant"}
      <div class="ml-turn ml-turn-agent" class:ml-error={isError(m.content)}>
        <div class="ml-name ml-name-agent">{plugin.i18n.t("chat.agent")}</div>
  ...
  {:else if busy}
    <div class="ml-turn ml-turn-agent ml-thinking-turn">
      <div class="ml-name ml-name-agent">{plugin.i18n.t("chat.agent")}</div>
      <div class="ml-thinking" aria-live="polite" aria-label={plugin.i18n.t("chat.thinking")}>
        <span class="ml-dot"></span><span class="ml-dot"></span><span class="ml-dot"></span>
      </div>
    </div>
  {/if}
